...
Position::Entry_tt::Entry_tt(int h_cost, const Position& position, const Card* candidate_homecell_next) noexcept : m_h_cost(h_cost), m_is_decided(false) {
...
void Position::Entry_tt::set_h_cost(int h_cost) noexcept {
    assert((h_cost >= 0) && (h_cost < 256));
    m_h_cost = h_cost;
    assert(correct());
}

void Position::Entry_tt::set_is_decided(bool is_decided) noexcept {
    assert(! m_is_decided);
    m_is_decided = is_decided;
    assert(correct());
}

int Position::Entry_tt::get_h_cost() const noexcept {
    assert(correct());
    return m_h_cost;
}

bool Position::Entry_tt::get_is_decided() const noexcept {
    assert(correct());
    return m_is_decided;
}

const Card* Position::Entry_tt::get_candidate_homecell_next() const noexcept {
    assert(correct());
    return m_candidate_homecell_next;
}

#ifdef TEST_ZKEY
bool Position::Entry_tt::identify(const Position& position) const noexcept {
    assert(correct());
    return m_union_array_card_below.identify(position.m_union_array_card_below);
}
#endif

bool Position::Action::correct() const noexcept {
    if(m_from == m_to) 
        return false;
    if((m_to >= 12) && (m_to <= 15)) 
...
int Position::Action::get_from() const noexcept {
    assert(correct());
    return m_from;
}

int Position::Action::get_to() const noexcept {
    assert(correct());
    return m_to;
}

bool Position::Action_for_h::correct() const noexcept {
    if(! (m_card.is_card()))
        return false;
...
const Card& Position::Action_for_h::get_card() const noexcept {
    assert(correct());
    return m_card;
}

int Position::Action_for_h::get_from() const noexcept {
    assert(correct());
    return m_from;
}

int Position::Action_for_h::get_to() const noexcept {
    assert(correct());
    return m_to;
}
...
int Position::find_freecell_empty() const noexcept {
	int i;
...
int Position::find_column_empty() const noexcept {
    int i;
...
Card Position::obtain_top_in_homecell(const Card& card) const noexcept {
    if(card.rank() == 0) 
        return Card::homecell();
    return Card(card.suit(), card.rank() - 1); 
}

Card Position::obtain_Card(char c) const noexcept {
    int id = (int)c;

    if(id <= 12) 
        return Card(Card::spade, id);
    if(id <= 25) 
        return Card(Card::club, id - 13);
    if(id <= 38) 
        return Card(Card::diamond, id - 26);
    if(id <= 51) 
        return Card(Card::heart, id - 39);
    if(id == Card::homecell_id) 
        return Card::homecell();
    if(id == Card::freecell_id) 
        return Card::freecell();
    if(id == Card::field_id) 
        return Card::field();
    return Card(); 
}
...
void Position::print() const noexcept {
    cout << "----------HOME_CELL----------" << endl;
    for(int suit=0; suit<SUIT_SIZE; ++suit) {
        Card card = m_array_homecell[suit];
        if(! card)
            cout << setw(4) << string(&Card::str_suits[suit], 1) + "0";
        else {
            assert(card.is_card());
            cout << setw(4) << card.gen_str();
        }
    }
    cout << endl;
    ...
    for(int column=0; column<TABLEAU_COLUMN_SIZE; ++column) {
        ...
        for(int i=nstack-1; i>=0; --i)
            cout << setw(4) << stack[i].gen_str();
        cout << endl;
    }
}

void Position::print_for_h() const noexcept {
    cout << "----------HOME_CELL----------" << endl;
    for(int suit=0; suit<SUIT_SIZE; ++suit) {
        Card card = m_array_homecell[suit];
        if(! card)
    ...
    cout << "----------FREECELL----------" << endl;
    Bits bits_freecell = m_bits_freecell;
    for(Card card=bits_freecell.pop(); card; card=bits_freecell.pop()) {
        assert(card.is_card());
        cout << setw(4) << card.gen_str();
    }
    for(int i=0; i<FREECELL_SIZE-m_ncard_freecell; ++i)
        cout << setw(4) << "-";
    cout << endl;
...
void Position::print_location() const noexcept {
    cout << "----------HOME_CELL----------" << endl;
    for(int suit=0; suit<SUIT_SIZE; ++suit) {
        Card card = m_array_homecell[suit];
        if(! card)
            ...
            assert(card.is_card());
            cout << setw(4) << (int)m_array_location[card.get_id()];
        }
    }
    cout << endl;
...
#endif
        for(int i=nstack-1; i>=0; --i)
            cout << setw(4) << (int)m_array_location[stack[i].get_id()];
        cout << endl;
    }
}
...
void Position::back_to_parent(const Position::Action_for_h* history, int naction) noexcept {
